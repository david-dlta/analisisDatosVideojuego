...
# Objetivo
El documento se engloba dentro de una serie de documentos de análisis de datos de videojuegos.
...
```{r}
covSessRev <- datos_ej1 %>%
  select(ID_USER, ID_SESSION,GAME_TIME,REVENUE) %>%
  ggplot() +
  geom_point(aes(x=GAME_TIME/1000, y=REVENUE))
```

No podemos observar una relación directa entre el tiempo jugado y los ingresos que el juego genera.
...
```{r}
covDayGameTime <- datos_ej1 %>%
  select(INSTALL_DATE,SESSION_DATE,GAME_TIME,COMPLETED_LEVELS) %>%
  ggplot() +
  geom_point(aes(x=GAME_TIME/1000, y=COMPLETED_LEVELS))
```

```{r}
covLevGameTime <- datos_ej1 %>%
